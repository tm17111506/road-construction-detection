'''
This file sets up the train and validation data according to the detectron format
'''
from nuscenes.nuscenes import NuScenes
from export_2d_annotations_as_json_mod import get_2d_boxes
import os
import json
import cv2
import numpy as np
...
def get_annotations(sample_data_token):
    annotations_2d = get_2d_boxes(nusc, sample_data_token, visibilities)
    annotations = []
    for anno_data in annotations_2d:
        ann = {}
        ann['bbox'] = anno_data['bbox_corners']
        ann['bbox_mode'] = 0 # detectron2.structures.BoxMode.XYXY_ABS
        ann['category_id'] = category_dict[anno_data['category_name']]
        annotations.append(ann)
    return annotations

n = len(nusc.scene)
for i, scene in enumerate(nusc.scene):
    print("Scene {}/{}: {}".format(i, n, scene['name']))
    scene_name = scene['name']
    is_train = scene_name in train
    token = scene['first_sample_token']
    while (len(token) > 0):
        print("On token: {}".format(token))
        sample = nusc.get('sample', token)
        subsamples = []
        for k, v in sample['data'].items():
            sample_data = nusc.get('sample_data', v)
            if 'CAM' in k and sample_data['is_key_frame']:
                annotations = get_annotations(sample_data['token'])
                if len(annotations) == 0:
                    continue
                
                # Build current sample_data
                data = dict()
                data['file_name'] = os.path.join(dataroot, sample_data['filename']) # To remove new line
                im = cv2.imread(data['file_name'])
                if np.any(im) == None: # photo may be damaged
                    continue
                data['height'], data['width'] = im.shape[0], im.shape[1]
                data['image_id'] = sample_data['token']
